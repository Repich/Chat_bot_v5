...
- вкладку `Обучение`, чтобы увидеть какие auto-learned навыки созданы, какие
  переиспользовались, какие падали и какие автоблокированы;
...
Если learned skill несколько раз подряд возвращает ошибку или пустой результат,
агент помечает его как `auto_blocked`. Такой навык остается в файлах и отчете,
но не участвует в планировании. Откройте его из вкладки `Обучение`, проверьте
запрос и либо исправьте, либо удалите.
