...
- [Критерии MVP](docs/mvp_acceptance.md): обязательные инварианты,
...
- [Граница передачи данных в LLM](docs/llm_data_boundary.md): защищенный
  внутренний режим, явное включение внешней ChatGPT-модели и аудит без содержимого payload.
...
- [База знаний экземпляра](docs/architecture/instance_knowledge.md): снимки
  документации, provenance, свежесть, противоречия и граница доверия.
- [Эксплуатация WIIC-экземпляра](docs/wiic_instance_operations.md): отдельный
  запуск, синхронизация BWiki, локальный импорт и откат снимка.
...
Failure solver может использовать утвержденный внутренний OpenAI-compatible
endpoint. Внешний API разрешается тем же явным флагом, что и основной LLM:
